...
- **Round 1 of the diagnostic above wasn't a real test - it only
  borrowed Flyer's own pixel art into a hand-written draw routine, not
  Flyer's own actual code**: "お前ただキャラ差し替えただけじゃねえだ
  ろうな Flyerを4体並べてスポーンさせんだよ キャラ変えただけでなんの
  検証になるんだよ 色も違うし 64x64まとめて処理するとダメな可能性が
  あるかもって言うテストなんだよ なら1体ずつ処理しての64x64ではどう
  なるかって言うことだ". Correct - `DRAW_FLUSH_ONE_BLOCK` computed
  Y/X/pattern itself and used `BOSS_COLOR`, never touching Flyer's own
  `UOFL_DRAW`/`FLYER_COLOR` at all, which is exactly why the color was
  wrong - a tell that it wasn't really Flyer's own code running.
  Rewritten properly: new `BOSS_FLYER_POOL` (4x `FLYER_SLOT_SIZE`, its
  own separate RAM - NOT the real `FLYER_POOL`/`FLYER_SLOT_COUNT`=1, so
  ordinary early-game Flyer spawning stays completely untouched) holds
  4 genuine Flyer-shaped instances, spawned together at `BOSS_SPAWN_TICK`
  positioned into the same 2x2 arrangement as before. Every frame,
  `UBA_UPDATE_REAL_FLYERS` processes them ONE AT A TIME in sequence -
  `CALL UPDATE_ONE_FLYER` (the real, completely unmodified routine -
  real cruise/home/exit AI, and it draws itself via its own real
  `UOFL_DRAW`/`UOFL_HIDE`, so `FLYER_COLOR`/`PAT_FLYER`/`PAT_FLYER_L`
  all resolve exactly like a genuine Flyer would, not reimplemented),
  then `CALL FLUSH_ONE_REAL_FLYER` for just that one instance's own 16
  bytes to its own dedicated hw slot group - before moving to the next
  instance. All 4 instances share `SPRIDX`=0 (always) so each one lands
  in the same `FLYER_SPRITE_ATTRS` staging spot in turn; safe
  specifically because each is flushed before the next one's own
  `UPDATE_ONE_FLYER` call overwrites it - genuinely "1体ずつ処理して".
  `UPDATE_BOSS_ALL`'s own former hand-written X/`BOSS_DIR` bounce-patrol
  is gone for this build - Flyer's own real AI drives each instance
  instead (cruise left, home toward the tank from wherever that
  instance's own Y happens to be, exit right, despawn - not a forever
  loop like the real boss's own patrol, since that's not what real
  Flyer behavior does; a real, disclosed side effect of using the
  actual subsystem unmodified rather than fighting it into staying
  glued together forever).
  A subtle register bug caught before ever running it for real: the
  first draft clobbered its own table-walk pointer (`HL`) across the 2
  calls per instance (`UPDATE_ONE_FLYER` and `FLUSH_ONE_REAL_FLYER`
  both freely reuse `HL` internally) - fixed by reading each instance's
  own hw-slot-base into a scratch RAM byte (`BOSS_FLYER_CUR_SLOT`)
  before either call, with `HL`/`BC` `PUSH`/`POP`-protected around both
  calls together. Also caught by the assembler itself, not by
  reasoning: `LD DE,FLYER_SLOT_SIZE : ADD IX,DE` isn't a supported form
  (`ADD IX,DE` - this same limitation is already documented elsewhere
  in this very file, "this assembler has no ADD IX,DE") - replaced with
  11x `INC IX` (`FLYER_SLOT_SIZE`), the same idiom every other pool
  loop here already uses.
  Verified: rendered a real frame shortly after spawn (`GAME_TICK`~1001)
  - 4 CYAN (`FLYER_COLOR`, correct this time) Flyer-art bodies, tiled
  into the same 2x2/64x64 arrangement, each instance's own state
  (`ACT`=1, `X`/`Y` matching its own quadrant offset, `PHASE`=0/cruise)
  confirmed directly from `BOSS_FLYER_POOL`'s own RAM. `boss_test.py`
  itself is now substantially stale for this build (most of its checks
  target the round-1 `BOSS_X`/`BOSS_DIR`/pattern-load state that no
  longer exists) and left unrewritten - a throwaway diagnostic state
  isn't worth investing full test coverage in; the real verification
  for this round is the direct RAM/render check above.
  **Whether this genuinely avoids the reported tearing can only be
  judged on real hardware/WebMSX**, same caveat as round 1 -
  `z80emu.py` has no interrupt simulation.
