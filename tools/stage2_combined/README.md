...
- **White hit-flash on every HP-bearing entity + the tank, plus 2 new
  enemies (Etank, Flyer), plus BigZum's HP retuned 8->5** - the full
  original request, with 2 attached sprite JSONs (`Flyer_32x32.json`
  fg7/bg1, `Etank_32x32.json` fg8/bg1):
  ...
  >
  > Etankは戦車で地上用
  > キャラは32x32のうち左下24x16ドットの範囲
  > 地形上り下りはしないので長い平地のみスポーン
  > 速度はZakoII緑と同じで
  > 合わせて通過に必要な長い平地を設置
  >
  > 右から出て左へ消える
  ...
  A pattern-code budget check partway through (Tank 32 groups + ZacoII
  2 + Explosion 1 + BulletU 2 + Zum 2 + BigZum 16 = 55 of the hardware's
  own 64-group ceiling used, only 9 free - Etank(2)+Flyer(8) needed 10)
  was raised via a question rather than guessed at, and the answer
  reshaped the Etank side of the design:
  > BigZum出現時は出さないので動的に書き換え
  > Flyerは頻出するのでそっちで確保
  > BigZumが出たら書き換え
  > キャラ位置は３２ｘ３２の内左下２４ｘ２４な
  > カラーはダークレッド

  - **Hit-flash** (`FLASH_COLOR`=15/white, `FLASH_DURATION`=6 frames,
    both untuned/inferred - "一瞬" only implied "short", no number):
    every HP-bearing entity's own draw routine now resolves a per-slot
    flash countdown into its color attribute(s) before writing them,
    decrementing once per drawn frame - ZacoII red reuses its own idle
    `E_DY` field (same "idle-field reuse" precedent as `E_DX` already
    doubling as its HP counter); BigZum grew `BIGZUM_SLOT_SIZE` 12->13
    for a new `+12` field (still fits the old 2-slot-sized 24-byte
    reservation even at 1 active slot, no address renumbering needed);
    Etank/Flyer include it in their own new slot layouts from the
    start. Triggered on every non-lethal damaging hit (`CHECK_HIT_PAIR`/
    `CHECK_HIT_PAIR_BIGZUM`/`CHECK_HIT_PAIR_ETANK`/`CHECK_HIT_PAIR_
    FLYER`'s own surviving-hit branches) - a lethal hit skips it and
    explodes instead, nothing to flash. The tank itself has no HP/
    damage system anywhere in this codebase (same gap `BIGZUM_PUNCH_
    KNOCKBACK`'s own comment already flagged), so its only discrete
    "took damage" moment is BigZum's own punch connecting
    (`UPDATE_TANK_BIGZUM_PUNCH`) - an inference, not confirmed with the
    user; Zum's/Etank's own push is a smooth continuous shove with no
    single hit instant, so nothing sets it from there.
  - **Etank**: a new ground "戦車" enemy, `ETANK_POOL`/`UPDATE_ETANK_
    ALL`/`ALLOC_ETANK_SLOT`/`UPDATE_ONE_ETANK`/`CHECK_HIT_PAIR_ETANK`/
    `UPDATE_TANK_ETANK_PUSH`, closely mirroring Zum's own shape but
    simpler (no terrain-following, no front/rear invincibility rule -
    neither was specified for Etank the way both were for Zum/BigZum,
    so `CHECK_HIT_PAIR_ETANK` is a plain omnidirectional HP-decrement
    instead, ZacoII red's own shape). Never follows terrain elevation
    at all - `ETANK_Y` is set once at spawn (from whichever tier the
    terrain probe found, always tier-index0 given the gate below) and
    never re-probed, so `ETANK_TERRAIN_OK` is stricter than Zum's own
    relaxed "any flat tier" check: it requires the CURRENT surface to
    specifically be tier-index0 (`IDCACHE_T0`), the only tier `terrain_
    gen.py`'s own `build_track()` was widened to a guaranteed-long run
    for (see the terrain entry below) - any other momentarily-flat tier
    is deliberately rejected even though Zum/BigZum would both happily
    spawn there. Speed reuses green ZacoII's own formula directly
    (`ENEMY_SPEED_LO + (TICK&1)`, not a `CALL ENEMY_GET_STEP` - that
    routine reads offsets that mean something different in `ETANK_
    POOL`'s own layout) - "速度はZakoII緑と同じで". Pushes the tank
    exactly like `UPDATE_TANK_ZUM_PUSH` (`ETANK_PUSH_SPEED EQU ZUM_
    PUSH_SPEED`), suspended while `JUMP_ACTIVE` so the 24x24 collision
    box stays jumpable, same reasoning as Zum's own. HP 10, spawns off
    the right edge, despawns at X=0 (no despawn-margin, same "reaches
    the true edge" precedent as Zum). Singleton (`ETANK_SLOT_COUNT`=1)
    - not specified either way, mirrors BigZum's own "1体のみ"
    convention rather than Zum's explicit 2-wide allowance (which WAS
    given directly for Zum specifically), both reading as the "one
    large tank" class of enemy here.
    - **Collision box corrected mid-round**: the original request's own
      literal "左下24x16" (bottom-left 24x16, matching the real art -
      confirmed directly: TL/TR are both fully blank in `sprites/
      Etank.json`) was overridden by the follow-up answer to "キャラ
      位置は３２ｘ３２の内左下２４ｘ２４な" - `ETANK_COLLISION_SIZE`=24/
      `ETANK_COLLISION_HEIGHT`=24/`ETANK_COLLISION_Y_OFFSET`=32-24=8,
      deliberately taller than the raw art bounds, same "collision
      doesn't have to match the art" precedent `BIGZUM_COLLISION_SIZE`/
      `HEIGHT` already established.
    - **Color corrected mid-round**: "カラーはダークレッド" overrides
      `sprites/Etank.json`'s own fg(8, medium red) - `ETANK_COLOR EQU 6`
      (dark red, MSX palette index 6), not the JSON's own value.
    - **No permanent pattern-code allocation - dynamic VRAM sharing
      with BigZum instead** ("BigZum出現時は出さないので動的に書き換え
      ...BigZumが出たら書き換え"): `etank_gen.py` (new) emits only
      Etank's own BL/BR quadrant bytes (TL/TR are blank, never drawn -
      those 2 hw sprite slots just stay hidden). `ALLOC_ETANK_SLOT`
      `LDIRVM`s those 64 bytes into `PAT_BIGZUM`'s own BL/BR groups
      (codes158-159, aliased here as `PAT_ETANK_BL`/`PAT_ETANK_BR`) at
      spawn time, safe only because the 2 are already mutually
      exclusive (`ALLOC_ETANK_SLOT` refuses to spawn while `BIGZUM_
      POOL`'s own ACT is nonzero, same "BigZum出現中にZumは出さないで
      くれ" precedent `ALLOC_ZUM_SLOT` already uses, just the other
      direction). `ALLOC_BIGZUM_SLOT` now reloads the real 128-byte
      BigZum region back into VRAM on every one of ITS OWN spawns (the
      same `LDIRVM` INIT already runs once at boot, just re-run per
      spawn now) so a BigZum appearing after an Etank did never shows
      stale Etank art. Verified directly: forcing an Etank spawn writes
      non-trivial bytes into that VRAM region; a subsequent forced
      BigZum spawn (after clobbering that region with garbage first)
      restores the exact original bytes.
    - **Terrain widened for a full crossing** ("合わせて通過に必要な
      長い平地を設置"): see the `terrain_gen.py` entry below - a ground
      enemy that never elevation-follows needs its ENTIRE crossing on
      flat ground, not just the spawn column.
  - **Flyer**: a new flying enemy, `FLYER_POOL`/`UPDATE_FLYER_ALL`/
    `ALLOC_FLYER_SLOT`/`UPDATE_ONE_FLYER`/`CHECK_HIT_PAIR_FLYER`, its
    own new `flyer_gen.py` (mirrors `bigzum_gen.py`'s structure, 1 pose
    instead of 2, `BASE_OFFSET`=220 right after BigZum's own last group
    at 219) getting a PERMANENT pattern allocation (unlike Etank) since
    it appears frequently and isn't gated behind any other enemy's
    absence - "Flyerは頻出するのでそっちで確保". 3-phase movement
    tracked in `+8 PHASE`: 0=cruise (straight left at a fixed height/
    speed, both untuned/inferred - `FLYER_CRUISE_Y`=64, `FLYER_SPEED`=2
    - no numbers were given, only the shape of the path) until it hits
    the left edge; 1=home (reverses, turns to face right, steps
    diagonally toward the tank's own current X/Y every frame via a
    signed compare+step on both axes - a literal "自機に向かってくる",
    not just a rightward cruise) until its own X passes the tank's;
    2=exit (straight right, ignoring the tank from then on - continuing
    to home in could otherwise oscillate forever once level with it)
    until off the right edge, then despawns. HP 4, omnidirectionally
    vulnerable (same "nothing specified either way" reasoning as
    Etank's own rule), full 32x32 collision box (no shrink - unlike
    BigZum/Etank, nothing shrank Flyer's own box, and it's not a jump-
    over/stand-on obstacle). Genuinely concurrent (`FLYER_SLOT_COUNT`
    =2, same 2-wide convention as Zum's own explicit allowance) with a
    shorter spawn interval than Zum/BigZum's own 90
    (`FLYER_SPAWN_INTERVAL`=60, untuned/inferred) so it actually shows
    up noticeably more often, matching "頻出する". No push/collision
    effect on the tank at all - unlike Etank/Zum, nothing described
    Flyer that way.
  - **BigZum HP**: `BIGZUM_HP_INIT` 8->5 ("合わせてBigZum耐久値5に変
    更").
  - **`terrain_gen.py`**: the tier-3(apex)/`IDCACHE_T0`-solid flat run
    Etank's own spawn gate targets existed at 2 separate points in the
    whole track (the widened one, and a 2nd, shorter one from the
    track's own "rapid back-to-back climb" section near the end) -
    since the runtime gate can only ask "is the CURRENT surface tier0",
    not "which of the 2 occurrences is this", BOTH needed widening or
    Etank could still spawn into the short one and run out of matching
    flat ground mid-crossing. Both widened to the same 92-cell/736px
    length (verified directly via a column-run scan of the generated
    `ROWS` arrays - was 48/24 cells respectively before).
  - New RAM (`ETANK_POOL`..`FLYER_DRAW_COLOR`, 0F340h-0F389h) grew past
    the old `STACKTOP`(0F380h) - moved to 0F390h, a few bytes of margin
    above the last used address.
  - New hw sprite slots: Etank 20-21 (2, BL/BR only), Flyer 22-29 (2
    instances x4) - total hw sprite usage now 0-29 of the hardware's
    own 32-slot ceiling.
  - Not confirmed with the user, flagged as inferences: the tank's own
    flash trigger (BigZum punch only); Etank's/Flyer's own omnidirect-
    ional (not front/rear) bullet-hit rule; Flyer's cruise/homing speed
    and cruise height; Flyer having no tank-collision effect; Etank's
    singleton concurrency limit; `ETANK_SPAWN_INTERVAL`/`FLYER_SPAWN_
    INTERVAL`'s own untuned magnitudes.

  Verified: a 29-case targeted test suite (isolated-subroutine calls,
  same "push a sentinel return address, jump straight into the
  routine" technique used earlier this session for BigZum) covering
  every piece above - `BIGZUM_HP_INIT`=5; `ALLOC_ETANK_SLOT`'s own
  3-condition gate (BigZum-active refusal, tier0-only terrain check
  including a climb-marker-id rejection, spawn success + non-trivial
  VRAM write); `ALLOC_BIGZUM_SLOT`'s own VRAM restore + HP=5 init;
  `UPDATE_TANK_ETANK_PUSH`'s contact push + `JUMP_ACTIVE` suspension;
  `CHECK_HIT_PAIR_ETANK`'s non-lethal HP-decrement+flash vs. lethal
  explode; `CHECK_HIT_PAIR_BIGZUM`'s own flash-on-damage; `UPDATE_TANK_
  BIGZUM_PUNCH`'s own tank-flash trigger; all 3 Flyer phase transitions
  (cruise->home at the left edge, home->exit once past the tank's X,
  exit->despawn off the right edge) plus its own homing X/Y step and
  FACING flip; `CHECK_HIT_PAIR_FLYER`'s non-lethal HP-decrement+flash -
  all 29 passed. Plus ZacoII red's own flash (non-lethal hit sets `E_
  DY`, `UOE_DRAW` renders `FLASH_COLOR` while it's nonzero and ticks it
  down) and the tank's own `UPDATE_TANK_SPRITES` flash override,
  checked the same way. A forced-spawn render (both new enemies placed
  in view via the real `ALLOC_*` routines, not hand-poked slots) shows
  Etank as a small dark-red tank shape sitting flush on the ground and
  2 cyan Flyers in the sky, both rendering cleanly with no corruption.
  Full regression: 20000-frame random-input sweep (`sim_dir`/`sim_trig_
  a`/`sim_trig_b`, not raw `JOY_*` pokes) with no crash/stall - Etank
  reached both its alive and (via the isolated test) exploding states,
  hit-flash observed on Etank/BigZum/Flyer, Etank never active
...
- `combined_test.asm`, `build_test.py` - the merged engine + build
  ...
  `enemy_gen.py`, `bigzum_gen.py`, `etank_gen.py`, and `flyer_gen.py`).
...
- `bigzum_gen.py`, `sprites/BigZum.json`, `sprites/BigZumP.json` -
  ...
  BigZum entry above).
- `etank_gen.py`, `sprites/Etank.json` - Etank's own BL/BR-quadrant-
  only conversion (TL/TR are blank, never drawn); no permanent pattern
  allocation, only ever loaded into VRAM dynamically at Etank's own
  spawn time (see the Etank/Flyer entry above).
- `flyer_gen.py`, `sprites/Flyer.json` - Flyer's full 32x32 (all 4
  quadrants, both facings) hw-pattern conversion, same `bigzum_gen.py`-
  style structure, permanently loaded at INIT.
