...
check("SBEAM_SPR_BASE_SLOT reuses the boss's own 16 body-quadrant slots "
      ...
      "reuse of ZacoII/BulletU)",
      SBEAM_SPR_BASE_SLOT == BOSS_SPR_BASE_SLOT)
...
# ---- FIRE_SBEAM: gated on BOSS_POSE_COUNT>=SBEAM_POSE_GATE ("ホーミング
# とサンダー2セット終わったら" - INFERRED as 2 completed poses) ----
cpu = fresh_cpu()
set_terrain_flat(cpu, 0)
cpu.mem[BOSS_POSE_COUNT] = SBEAM_POSE_GATE - 1
call_routine(cpu, "FIRE_SBEAM")
check("FIRE_SBEAM is a silent no-op before BOSS_POSE_COUNT reaches SBEAM_POSE_GATE",
      cpu.mem[SBEAM_ACT] == 0)

cpu = fresh_cpu()
set_terrain_flat(cpu, 0)
cpu.mem[BOSS_POSE_COUNT] = SBEAM_POSE_GATE
call_routine(cpu, "FIRE_SBEAM")
check("FIRE_SBEAM arms the drop phase (ACT=1) once BOSS_POSE_COUNT reaches SBEAM_POSE_GATE",
      cpu.mem[SBEAM_ACT] == 1)
...
# sweep phase rendering: unified column>=FRONT_COL formula, including the
# real 16-slot hw cap (columns below SBEAM_START_COL-15 never get a
# sprite even though FRONT_COL itself keeps tracking down to 0)
cpu = fresh_cpu()
cpu.mem[SBEAM_ACT] = 2
cpu.mem[SBEAM_FRONT_COL] = 20
cpu.mem[SBEAM_GROUND_Y] = 152
cpu.mem[SBEAM_BLINK] = 1   # next toggle -> 0 -> the visible pass
call_routine(cpu, "STAGE_SBEAM")
all_correct = True
for i in range(SBEAM_SLOT_COUNT):
    col = SBEAM_START_COL - i
    s = slot(cpu, i)
    expect_visible = col >= 20
    if expect_visible != (s["y"] != 209):
        all_correct = False
    if expect_visible and (s["x"] != col * 8 or s["y"] != 152):
        all_correct = False
check("sweep phase: every slot's own visibility/position exactly matches the unified "
      "column>=SBEAM_FRONT_COL formula at a mid-sweep FRONT_COL", all_correct)
check("sweep phase: the hw 16-slot cap really does leave low columns (13 down to 0) "
      "without a sprite even once FRONT_COL reaches them - a genuine, flagged deviation "
      "from a literal unbounded 'スプライトを足していく'",
      SBEAM_START_COL - (SBEAM_SLOT_COUNT - 1) > 0)


# ---- real MAINLOOP: SBeam stays silent before the 3rd pose, then fires,
# completes a full drop/sweep/retract cycle, and never corrupts the
# boss's own body sprite once the pose ends ----
cpu = fresh_cpu()
cpu.sim_dir = 0
