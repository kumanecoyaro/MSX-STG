...
1回のstep_frame()呼び出しが演出中の1フレーム(TANK_ENTRY_ACT!=0の
JP MAINLOOPループ含む)に正確に対応する。
"""
import os
import sys
...
#         terrain-scroll/GAME_TICK-advance block) ----
...
# ---- 3. booster animation toggles every single frame ----
cpu3 = fresh_cpu(skip_intro=False)
anims = []
for _ in range(6):
    step_frame(cpu3)
    anims.append(cpu3.rd(TANK_ENTRY_ANIM))
check("TANK_ENTRY_ANIM alternates 0/1/0/1/... every frame (\"落下中は1と2を"
      "1フレ切り替え\")", anims == [1, 0, 1, 0, 1, 0])

# ---- 4. an independent Python simulation of the X/Y motion, tick-for-tick ----
...
check("TANK_X matches an independent Python parabola simulation tick-for-tick",
      xs_real == xs_exp)
check("TANK_Y_CUR matches an independent Python parabola simulation tick-for-tick",
      ys_real == ys_exp)

# landing frame: the first frame where BOTH axes have reached their target
...
# ---- 5. booster X clamps to 0 instead of underflowing while TANK_X<16 ----
cpu5 = fresh_cpu(skip_intro=False)
step_frame(cpu5)  # frame1: TANK_X=1 (< 16)
...
# ---- 6. booster position/Y-offset/pattern once TANK_X is comfortably >=16 ----
cpu6 = fresh_cpu(skip_intro=False)
for _ in range(20):
    step_frame(cpu6)
tank_x = cpu6.rd(TANK_X)
tank_y = cpu6.rd(TANK_Y_CUR)
assert tank_x >= 16, "test precondition: need TANK_X>=16 by frame 20"
...
# ---- 6b. the 16x16 sprite is a real hardware 4-consecutive-code quad (this
...
# ---- 7. the booster sprite is actually flushed to hw sprite ATTRIBUTE slot
...
# ---- 8. landing: TANK_ENTRY_ACT drops to 0 exactly on the frame both axes
...
# ---- 9. once landed, normal MAINLOOP processing genuinely resumes (TICK/
