...
def emit_asm_tables():
    """(2026-09-19、"あとはステージ2はキャラの反転が多いんで 1パターン
    から反転したデータをその場で生成してVramに書き込むように変更"):
    ENEMY_<NAME>_FLIP no longer carries its own DB data - combined_
    test.asm's INIT generates both ZacoII's and Zum's flipped pattern
    at boot time from ENEMY_<NAME> via MIRROR_16_TO_VRAM (a mechanical
    bit-reverse + quadrant-swap). Zum's flip IS actually read at
    runtime (UOZ_DRAW/CHPZ_ORIENT while Z_RETREAT==1 - a since-outdated
    comment in combined_test.asm claimed otherwise, verified and fixed
    when this change was made) so it still needs generating, just no
    longer needs its own ROM bytes. ALL_PATTERNS_FLIP itself stays
    available in this module for tests to cross-check against."""
