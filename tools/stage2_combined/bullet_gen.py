...
def emit_asm_tables():
    ...
    Vramに書き込むように変更"を検討したが、この差し戻しの通りBullet
    には不採用): Tank/BigZum/Flyer/ZacoII等の128byte/32byte単位の
    ポーズと違い、Bulletの各データは8byte(BulletF/U_BGのフラットな
    単一タイル)しかない。オンザフライ生成1箇所あたりのコード
    オーバーヘッド(VDP書き込みアドレス設定+CALL、約12-17byte)が
    節約できるデータ量(8-32byte、しかも複数の背景色グループへ同じ
    ソースを繰り返しロードするため実際に節約できるのは重複排除できる
    分だけ)を上回り、実測でROM容量が正味+128byte(30527→30655)と
    悪化することを確認した上でこの実装は撤回・元のLDIRVM方式へ
    復元済み(教訓: 「反転データの重複排除」はデータブロックが十分に
    大きい場合のみROM削減に有効、8byte単位の小さなタイルでは逆効果)。"""
