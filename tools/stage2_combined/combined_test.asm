...
BIGZUM_POOL       EQU 0F307h  ; BIGZUM_SLOT_SIZE*BIGZUM_SLOT_COUNT = 24 bytes reserved (only the first 12 actually used now - see BIGZUM_SLOT_COUNT)
BIGZUM_SPAWN_TIMER EQU 0F31Fh
...
BIGZUM_SPRITE_ATTRS EQU 0F320h   ; BIGZUM_SLOT_COUNT*16 = 32 bytes: (Y,X,pat,col)x4 per instance
BIGZUM_DRAW_TEMP  EQU 0F340h     ; scratch byte, UOBZ_DRAW's own chosen pattern base
BIGZUM_DRAW_COLOR EQU 0F341h     ; scratch byte, UOBZ_DRAW's own resolved color (BIGZUM_COLOR or FLASH_COLOR) - still well under the real 0F380h BIOS-work-area boundary (see STACKTOP's own comment)
...
; ends at 0F370h - well clear of the 0F380h boundary, no DRAW_TEMP/
...
; "Etank実装で根本的なバグがある" - found via careful re-derivation of
; UOBZ_DRAW's own actual quadrant addressing (TL=+0,TR=+4,BL=+8,BR=+12
; - each 16x16-mode hw sprite code needs its own 4-code group; every
; OTHER multi-pose/multi-quadrant entity in this file already follows
; this same 4-apart spacing - PAT_ZACO/_FLIP, PAT_BULLETU/_L, PAT_ZUM/
; _FLIP, BigZum's own 4 quadrants). PAT_BIGZUM+2/+3 (only 1 code apart,
; copied verbatim from the pre-rollback implementation without
; re-deriving it against the CURRENT UOBZ_DRAW) landed inside the
; MIDDLE of BigZum's own TL quadrant's own 4-code span (156-159) -
; Etank's own 64-byte dynamic-share LDIRVM (starting at that wrong
; base) corrupted BigZum's TL's own last 2 sub-tiles, all 4 of TR's,
; and BL's first 2, and Etank's own BL/BR hw sprites displayed a
; misaligned slice of the wrong bytes instead of a real quadrant -
; garbled art on Etank itself, and (until BigZum's own reload-on-spawn
; fix from a previous round ran) corrupted BigZum too. Corrected to
; BigZum's own real BL(+8)/BR(+12) quadrant group bases - still
; contiguous (164-171, BL's 4 codes then BR's 4), so the existing
; single 64-byte LDIRVM still covers both correctly with no other
; change needed.
