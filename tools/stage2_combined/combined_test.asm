...
INIT_SPRATR_CLR:
    ...
    LD (DASH_ACTIVE),A
    ; (2026-09-23follow-up11、"落下中の自機はジャンプ時のものに"): 落下演出
    ; 中はジャンプ時と同じPAT_TANKFGAP。TANK_ENTRY_FINISHでPAT_TANKFへ戻す。
    LD A,PAT_TANKFGAP : LD (CUR_POSE_PAT),A
    XOR A
    LD (SHOT_COOLDOWN),A
    LD (BULLET0_ACT),A
    LD (BULLET1_ACT),A
...
; 添付ファイルを参考に"): "MISSION COMPLETED"表示直前(CREDIT表示が
; ENDING_BLANK_ROW32で全幅ブランクされ二度と参照されなくなる瞬間)に、
; ending_text_gen.pyのCOMPLETE専用新フォント(tools/pixel_font_8x8.py
; ベース、8x8)をCREDIT用フォントと全く同じcode96-103+144-147へ上書き
; ロードする。色(group12/18、白文字/黒背景)は既にENDING_LOAD_FONTが
; ロード済みの値のまま流用可能(新フォントも同じ0F1h)。
ENDING_FINISH:
    ...
    LD HL,ENDING_COMPLETE_FONT_BLOCK0 : LD DE,ENDING_COMPLETE_FONT_BLOCK0_CODE*8 : LD BC,ENDING_COMPLETE_FONT_BLOCK0_LEN : CALL LDIRVM
...
; 鳴っていない」結果になった実体、前段の"chB駆動/chC追従"非対称設計は
; この制約への苦肉の策だった)。ここからは完全にソフトウェア側
...
; チャンネルB(R2/R3 tone、R9 volume、BELL形状+デューティ50%)の1tick
; 分の前進。音符の頭(NEWROW)でテーブルをindex0からリトリガー、継続
; tickは毎回テーブルを1段ずつ前進させてR9へ書く(HWエンベロープと違い
; 共有ジェネレータの制約が無いため、休符以外は常にPSGへ書いてよい)。
; LOOP_MARKに当たったらBGM_B_BASEへ戻ってその行を読み直す。BGM_TICKから
; 呼ばれる時点でAF/BC/DE/HLは既に保存済みなので、自由に使える。
BGMT_UPDATE_B:
    LD A,(BGM_B_TIMER)
    OR A
    JR Z,BGMT_UB_NEWROW
...
; チャンネルC(R4/R5 tone、R10 volume、LINEAR形状+デューティOFF)版 -
; 構造はchBと同型だが、デューティゲートを持たず毎tick常にエンベロープ
; 値をそのままR10へ書く。
BGMT_UPDATE_C:
    LD A,(BGM_C_TIMER)
    OR A
...
; to unconditionally at the end of this chain instead - see above)
    ...
    CP 2   : JP Z,ALLOC_BIGZUM_SLOT
    CP 3   : JP Z,SPAWN_S2_ZACOII
    CP 4   : JP Z,SPAWN_S2_ZACOII
    CP 5   : JP Z,SPAWN_S2_ZACOII
    CP 6   : JP Z,ALLOC_FLYER_SLOT
    CP 7   : JP Z,ALLOC_FLYER_SLOT
    ...
    CP 13  : JP Z,SPAWN_S2_ZACOII
    CP 14  : JP Z,SPAWN_S2_ZACOII_RED
    CP 15  : JP Z,ALLOC_ZUM_SLOT
    CP 16  : JP Z,ALLOC_ZUM_SLOT
    CP 17  : JP Z,ALLOC_FLYER_SLOT
    CP 18  : JP Z,ALLOC_ZUM_SLOT
    CP 19  : JP Z,SPAWN_S2_ZACOII
    CP 20  : JP Z,SPAWN_S2_ZACOII
    CP 21  : JP Z,SPAWN_S2_ZACOII
    CP 22  : JP Z,ALLOC_ETANK_SLOT
    ...
    CP 29  : JP Z,ALLOC_BIGZUM_SLOT
    CP 30  : JP Z,SPAWN_S2_ZACOII
    CP 31  : JP Z,SPAWN_S2_ZACOII
    CP 32  : JP Z,SPAWN_S2_ZACOII
    CP 33  : JP Z,ALLOC_FLYER_SLOT
    CP 34  : JP Z,ALLOC_FLYER_SLOT
    CP 35  : JP Z,ALLOC_ZUM_SLOT
    CP 36  : JP Z,SPAWN_S2_ZACOII_RED
    CP 37  : JP Z,ALLOC_ZUM_SLOT
    CP 38  : JP Z,SPAWN_S2_ZACOII
    ...
    CP 46  : JP Z,ALLOC_FLYER_SLOT
    CP 47  : JP Z,ALLOC_ZUM_SLOT
    CP 48  : JP Z,ALLOC_ZUM_SLOT
    CP 49  : JP Z,ALLOC_FLYER_SLOT
    ...
    CP 60  : JP Z,ALLOC_ETANK_SLOT
    CP 61  : JP Z,ALLOC_FLYER_SLOT
    ...
    CP 68  : JP Z,SPAWN_S2_ZACOII
    CP 69  : JP Z,SPAWN_S2_ZACOII_RED
    CP 70  : JP Z,ALLOC_BIGZUM_SLOT
    CP 71  : JP Z,SPAWN_S2_ZACOII
    CP 72  : JP Z,SPAWN_S2_ZACOII
    CP 73  : JP Z,SPAWN_S2_ZACOII
    CP 74  : JP Z,SPAWN_S2_ZACOII
    CP 75  : JP Z,SPAWN_S2_ZACOII
    CP 76  : JP Z,SPAWN_S2_ZACOII_RED
    CP 77  : JP Z,SPAWN_S2_ZACOII
    CP 78  : JP Z,SPAWN_S2_ZACOII
    CP 79  : JP Z,SPAWN_S2_ZACOII
    CP 80  : JP Z,SPAWN_S2_ZACOII
    CP 81  : JP Z,ALLOC_ZUM_SLOT
    CP 82  : JP Z,ALLOC_FLYER_SLOT
    CP 83  : JP Z,ALLOC_ZUM_SLOT
    CP 84  : JP Z,ALLOC_FLYER_SLOT
    CP 85  : JP Z,SPAWN_S2_ZACOII
    CP 86  : JP Z,SPAWN_S2_ZACOII
    ...
    CP 94  : JP Z,SPAWN_S2_ZACOII_RED
    CP 95  : JP Z,ALLOC_FLYER_SLOT
    CP 96  : JP Z,SPAWN_S2_ZACOII
    CP 97  : JP Z,ALLOC_FLYER_SLOT
    ...
    CP 105 : JP Z,SPAWN_S2_ZACOII_RED
    CP 106 : JP Z,SPAWN_S2_ZACOII
    CP 107 : JP Z,SPAWN_S2_ZACOII
    CP 108 : JP Z,SPAWN_S2_ZACOII
    CP 109 : JP Z,SPAWN_S2_ZACOII
    CP 110 : JP Z,SPAWN_S2_ZACOII
    CP 111 : JP Z,ALLOC_ETANK_SLOT
    CP 112 : JP Z,SPAWN_S2_ZACOII
    CP 113 : JP Z,SPAWN_S2_ZACOII
    CP 114 : JP Z,SPAWN_S2_ZACOII
    ...
    CP 123 : JP Z,ALLOC_FLYER_SLOT
    CP 124 : JP Z,SPAWN_S2_ZACOII
    CP 125 : JP Z,ALLOC_BIGZUM_SLOT
    CP 126 : JP Z,ALLOC_FLYER_SLOT
    CP 127 : JP Z,SPAWN_S2_ZACOII
    CP 128 : JP Z,ALLOC_FLYER_SLOT
    CP 129 : JP Z,ALLOC_FLYER_SLOT
    CP 130 : JP Z,SPAWN_S2_ZACOII
    CP 131 : JP Z,SPAWN_S2_ZACOII_RED
    ...
    CP 141 : JP Z,SPAWN_S2_ZACOII
    CP 142 : JP Z,SPAWN_S2_ZACOII_RED
    CP 143 : JP Z,SPAWN_S2_ZACOII
    CP 144 : JP Z,ALLOC_ZUM_SLOT
    CP 145 : JP Z,SPAWN_S2_ZACOII
    CP 146 : JP Z,ALLOC_ZUM_SLOT
    CP 147 : JP Z,SPAWN_S2_ZACOII
    CP 148 : JP Z,ALLOC_ZUM_SLOT
    CP 149 : JP Z,SPAWN_S2_ZACOII
    CP 150 : JP Z,SPAWN_S2_ZACOII
    CP 151 : JP Z,SPAWN_S2_ZACOII
    CP 152 : JP Z,ALLOC_FLYER_SLOT
    CP 153 : JP Z,ALLOC_FLYER_SLOT
    CP 154 : JP Z,ALLOC_ETANK_SLOT
    CP 155 : JP Z,SPAWN_S2_ZACOII
    CP 156 : JP Z,SPAWN_S2_ZACOII
    CP 157 : JP Z,SPAWN_S2_ZACOII
    CP 158 : JP Z,SPAWN_S2_ZACOII
    CP 159 : JP Z,SPAWN_S2_ZACOII
    CP 160 : JP Z,SPAWN_S2_ZACOII
    CP 161 : JP Z,ALLOC_BIGZUM_SLOT
    CP 162 : JP Z,SPAWN_S2_ZACOII
    CP 163 : JP Z,SPAWN_S2_ZACOII
    CP 164 : JP Z,SPAWN_S2_ZACOII
    CP 165 : JP Z,SPAWN_S2_ZACOII_RED
    CP 166 : JP Z,SPAWN_S2_ZACOII
    CP 167 : JP Z,ALLOC_FLYER_SLOT
    CP 168 : JP Z,ALLOC_FLYER_SLOT
    CP 169 : JP Z,SPAWN_S2_ZACOII
    CP 170 : JP Z,SPAWN_S2_ZACOII
    CP 171 : JP Z,ALLOC_FLYER_SLOT
    CP 172 : JP Z,ALLOC_FLYER_SLOT
    CP 173 : JP Z,SPAWN_S2_ZACOII
    CP 174 : JP Z,SPAWN_S2_ZACOII
    CP 175 : JP Z,SPAWN_S2_ZACOII_RED
    JP S2_BOSS_SPAWN

SPAWN2_COUNT EQU 177

SPAWN2_THRESHOLDS:
    DW 12,15,23,27,34,46,57,67
    DW 78,80,84,95,98,100,110,124
    DW 139,145,155,157,160,164,169,182
    DW 185,196,207,209,209,218,220,221
    DW 225,232,242,259,260,263,280,284
    DW 292,295,298,307,309,310,316,321
    DW 325,326,334,335,344,355,355,355
    DW 362,363,363,372,381,386,395,397
    DW 401,403,407,411,413,428,429,463
    DW 466,469,472,475,486,494,500,506
    DW 506,512,517,517,524,535,535,539
    DW 546,546,550,555,556,560,565,577
    DW 580,587,591,597,604,604,604,613
    DW 613,622,633,635,639,647,649,659
    DW 675,677,682,684,688,691,693,697
    DW 700,701,705,712,720,724,725,736
    DW 736,747,748,760,768,770,774,777
    DW 779,783,784,786,789,798,808,816
    DW 826,832,835,837,841,842,849,855
    DW 866,869,869,877,878,882,885,887
    DW 890,894,897,898,902,908,914,915
    DW 920,924,928,928,932,935,938,942
    DW 995

SPAWN2_Y_TABLE:
    DB 104,56,184,88,32,72,80,40,104,72,24,112
    DB 80,40,72,144,144,48,144,88,64,24,128,72
    DB 40,72,88,24,56,128,88,64,24,80,48,144
    DB 48,144,88,56,96,64,24,104,72,32,72,144
    DB 144,32,88,16,64,24,48,80,32,64,96,80
    DB 128,32,80,24,104,64,16,96,48,40,128,72
    DB 24,72,16,72,48,72,8,40,120,168,80,168
    DB 32,64,88,24,56,88,24,96,56,24,56,88
    DB 32,32,104,56,24,72,112,40,80,64,104,64
    DB 24,72,32,128,88,56,88,56,24,96,56,24
    DB 104,64,32,80,64,128,24,72,88,40,16,64
    DB 104,72,32,104,64,24,104,72,32,80,72,32
    DB 144,72,144,56,144,24,80,40,88,48,128,104
    DB 72,40,112,80,40,128,120,88,40,72,24,88
    DB 32,80,16,88,32,120,24,64,104
...
TANK_ENTRY_FINISH:
    ...
    LD A,PAT_TANKF : LD (CUR_POSE_PAT),A   ; 着地: ジャンプ時の絵→通常の絵
