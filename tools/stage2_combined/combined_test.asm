...
; **注意点(ユーザーには提示済み・指示通り一旦このまま)**: #1は
; 「1回減衰しきったら音符の残りtickは無音のまま」という性質のため、
; EP(周期)を変えない限り約86ms(下記コメント)より長い音符は本来の
; 長さより早く無音になる - リズムを刻み続ける#5と違い、音符の長さと
; エンベロープ形状が根本的にトレードオフの関係にある。それでも
; 望んだ音にならない場合は"ソフトウェアに変える"という撤退条件が
; 既に与えられている。
