...
class Z80:
    ...
    def step(self):
        ...
        if op == 0x00: pass  # NOP
            ...
            self.halted = True
            return
        elif op == 0xF3: self.iff1=False  # DI
        elif op == 0xFB: self.iff1=True   # EI
        elif op == 0xC9:  # RET
            self.pc = self.pop()
        elif op in (0xC0,0xC8,0xD0,0xD8,0xE0,0xE8,0xF0,0xF8):  # RET cc
            cc = (op>>3)&7
            if self.check_cc(cc): self.pc = self.pop()
            ...
            target=self.fetch16()
            if self.check_cc(cc):
                self.push(self.pc); self.pc=target
        elif op == 0xC3:
        ...
        elif op == 0xC1: self.setbc(self.pop())
        elif op == 0xD1: self.setde(self.pop())
            ...
            dst=(op>>3)&7; src=op&7
            if dst==6:  # LD (HL),r
                self.wr(self.hl(), self.getr8(src))
            elif src==6:  # LD r,(HL)
                self.setr8(dst, self.rd(self.hl()))
            else:
                self.setr8(dst, self.getr8(src))
        elif (op & 0xC7) == 0x06:  # LD r,n
            dst=(op>>3)&7
            n=self.fetch()
            if dst==6: self.wr(self.hl(), n)
            else: self.setr8(dst,n)
    ...
    def step_dd(self):
        ...
        elif op == 0x23: self.ix=(self.ix+1)&0xFFFF
        elif op == 0x2B: self.ix=(self.ix-1)&0xFFFF
        elif op == 0xE5: self.push(self.ix)
        elif op == 0xE1: self.ix=self.pop()
    ...
    def step_fd(self):
        op=self.fetch()
        if op == 0xE5: self.push(self.iy)
        elif op == 0xE1: self.iy=self.pop()
        elif op == 0x21: self.iy=self.fetch16()
        else:
