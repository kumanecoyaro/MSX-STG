...
S1_FAIL_INIT:
    ...
    LD A,0D0h : OUT (98h),A
    PUSH BC : POP BC : NOP : NOP
    LD HL,S1_FONT : LD DE,S1_FONT_BASE*8 : LD BC,136 : CALL LDIRVM
    LD HL,S1_FONT_COLOR : LD DE,2008h : LD BC,3 : CALL LDIRVM   ; group8-10 白/黒
    DI
...
S1F_BLANK:
    ...
    ; (2026-09-23、"2つに分けずに You needs shield,and you needs enough energy
    ; だけで良いわ"): 理由(S1_FAIL_REASON)に関係なく条件を2行とも出す。
    LD DE,12*32+1800h+5 : LD HL,S1_MSG_SHIELD : CALL S1_PUT_MSG
    LD DE,14*32+1800h+4 : LD HL,S1_MSG_ENERGY : CALL S1_PUT_MSG
    CALL GO_INIT_BGM
    EI
    JP GO_WAIT_START
...
S1_MSG_SHIELD:
    DB 21
    DB 75,67,76,69,68,73,73,74,66,69,66,77,65,73,72,74,80,69,71,68,74   ; "YOU NEEDS SHIELD, AND"
S1_MSG_ENERGY:
    DB 23
    DB 75,67,76,69,68,73,73,74,66,69,73,68,67,76,78,77,69,73,68,73,79,78,75   ; "YOU NEEDS ENOUGH ENERGY"
