...
- **画面向き(ランドスケープ)固定の試み**: WebMSX自身はレスポンシブCSSで
  現在の向きに追従するだけで、Screen Orientation APIによる向きロックは
  一切行わない。itch.ioの「Mobile orientation」設定だけでは実機で
  「画面に触れた瞬間にポートレイトへ戻る/起動前に切り替えても効かない」との
  報告があったため、`</body>`直前に`screen.orientation.lock("landscape")`を
  試みる小さなスクリプトを追加した(`fullscreenchange`イベントの都度、および
  `load`時に再試行)。
  - **既知の制限**: このAPIはブラウザによっては「documentが実際に
    フルスクリーン状態であること」を要求するため、フルスクリーンに
    入る前は失敗しうる(`.catch(()=>{})`で無視、コンソールにも出さない)。
    **iOS Safariは本APIを2026-09時点で実装していない**ため、iPhone/iPadでは
    このロックは効かず、ユーザーが手動で端末を回転してレスポンシブ
    レイアウトに追従させる以外に確実な方法は無い(既存のWeb技術の制約であり
    itch.io側・本パッケージ側どちらでも回避できない)。Android Chrome等
    Screen Orientation APIをサポートするブラウザ+フルスクリーン中であれば
    動作するはず(実機検証はユーザー側にお願いしたい - このセッションの
    サンドボックス環境はheadless Chromiumかつ実タッチデバイスではないため、
    このAPI自体の実機での成否をこちらでは検証できていない)。
