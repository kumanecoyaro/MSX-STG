...
INIT_BGM:
    ...
    LD (7000h),A
    ; (2026-09-24、"絵柄データとローダーから逃がして"): Stage1が起動時にVRAMへ送る絵柄データと
    ; 転送の一覧表(src/CYBER SHMUP.asmのSTAGE1_GFX_RAM区画)を、このファイル自身のbank1に置いた
    ; STAGE1_GFX_BLOBからRAMへ。中身と長さはComb版でtools/bankswitch_poc/build_full_rom.pyが
    ; 差し替える(このファイル単体では1byteだけの置き場)。
    LD HL,STAGE1_GFX_BLOB : LD DE,STAGE1_GFX_RAM_T : LD BC,STAGE1_GFX_LEN : LDIR
...
; (2026-09-24) Stage1の起動時の絵柄データ(INIT_BGM参照)。build_full_rom.pyがComb用に差し替える。
STAGE1_GFX_RAM_T EQU 0D300h      ; src/CYBER SHMUP.asmのSTAGE1_GFX_RAMと一致させること
STAGE1_GFX_LEN   EQU 1
STAGE1_GFX_BLOB:
    DB 0
