...
## 実機ハードウェア制約(絶対厳守・全ファイル共通・Round番号に依存しない恒久ルール)

**このセクションはRound履歴の一部ではなく、常に最初に読むべき恒久ルールとして独立に置く。**
(経緯: 2026-09-06 Round47でOTIRバグを実機グリッチとして踏み、修正後にユーザーから
"またそれか OTIRは使うなと以前も指示してあったんだが 地形スクロールでデバッグが難航して
ヤバかったんだわ"との指摘を受けた。`HANDOFF.md`/`README.md`を"OTIR"以外にも
"地形スクロール"・"LDIR"・"INIR"・"OTDR"・"INDR"・"ブロック転送"等の関連語で横断的に
再検索したが、Round47で自分が書いた記述以外にこの禁止事項の記録は一件も見つからなかった
- つまり過去にこの指示を受けていたとしても、それはこのプロジェクトの参照可能などの記録
(HANDOFF.md/README.md/CLAUDE.md)にも残っていなかったことになる。記録の有無に関わらず、
同じ種類の実機専用バグ(z80emu.pyでは検出不可能)を2回踏ませてしまったこと自体が問題であり、
今後は「見出し埋没型のRound履歴」ではなく、常に最初に目に入るこの独立セクションで管理する。)

- **Z80のブロックI/O命令(`OTIR`/`OTDR`/`INIR`/`INDR`)は、MSXのハードウェアI/Oポート
  (VDPデータポート98h・VDPレジスタポート99h・PSGレジスタポート等)への連続転送に
  絶対に使用しないこと。** 通常の`OUT (n),A`/`IN A,(n)`を`DJNZ`等で回す手動ループのみを
  使うこと。理由: これらの命令の内部バスタイミングがVDP等の実ハードウェアの要求と
  合わないという既知のMSX/Z80ハードウェア制約があり、実機では書き込み/読み込みが
  実質無効化されるが、CPU側のカウンタ・ポインタは正常に進行したかのように振る舞うため、
  検出が非常に困難な形(1バイトもエラーを出さずに後続データが累積的にズレて画面全体が
  破損する等)で症状が現れる。
- **`z80emu.py`はこの制約を一切再現しない**(OTIR等は単なる論理ループとして実装されており、
  実行結果としては「正しく」動作してしまう)。したがって、この種のバグは
  `run_all.py`の全回帰テストが1件残らずPASSしていても実機・WebMSX・BlueMSX等では
  100%再現しうる。「テスト全緑なのに実機でのみ100%再現する不具合」に遭遇したら、
  まずこの種のBIOS/ハードウェアプロトコル依存(z80emu.pyがno-opスタブ化している範囲)を
  疑うこと(openMSX等のハードウェア精度の高いエミュレータでの実VRAM/実レジスタの
  バイト単位比較調査が有効、手法の詳細はHANDOFF.md Round47参照)。
- 新規にVRAM/PSG/その他ハードウェアポートへのブロック転送を実装する際は、着手前に
  必ずこのセクションを再確認し、`OTIR`系命令を使わないこと。

## Stage2 ROM予算(2026-09-25、follow-up49時点で**残り4961byte**)
...
## Stage1 ROM予算(2026-09-24、Round145 follow-up46時点で**plain 2797byte /
Comb 2747byte**・恒久的に確認必須、Combの方が少ない)
...
- **(2026-09-23、follow-up26)** ボスレーザーを連射の流れ化+干渉点/飛び散り/右端スプライト(16x24、
  ...
  (`JP WRTVRM`等)は使わず`CALL ...: RET`にすること。
...
- **(2026-09-23、follow-up23、調整用・一時的)** `build_full_rom.py`の
  ...
  verify_*.pyが使う`assemble_game()`は既定でdebug無し。**(follow-up30、"Tickを通常に")
  Falseに戻し済み**。再びボス調整が必要になったらTrueにするだけで使える。

- **(2026-09-23、follow-up22)** ボス専用レーザー+レーザー干渉+エナジー
  ゲージ(LZ_*/GAUGE_*、ファイル末尾)で744byte使用、960→216byte。
  次に大きな機能を足す場合は、タイトルバンク(bank1、約12KB空き)の
  コードをTitleがRAMへ事前コピーしてStage1からCALLする方式を検討すること。
...
- **Round145 follow-up追記**: スケジュール差し替え(Schedule_2_7.json)を
  ...
  超過)というComb限定の新パターン**に遭遇。従来の「新規コードを
  ファイル末尾へ移す」回避策を2回試したが**両方とも悪化**した(詳細は
  `tools/stage2_combined/HANDOFF.md`のRound145 follow-up参照) - これは
  「ファイル末尾移動」ヒューリスティックの初めての反例で、元の位置が
  すでにALIGN境界のパディング吸収の恩恵を受けていた場合、移動すると
  その恩恵ごと失われて悪化しうることが判明した。**この種の「Comb限定で
  ○○byte超過」に遭遇した場合、まず次の診断手順を踏むこと**:
  1. plainとCombの両方をアセンブルし、同一シンボル名のアドレスを
     ファイル前方から後方へ順に比較して`delta = comb_addr - plain_addr`
     を追跡する。
  2. deltaが一定値(例: 171)で安定している区間から、突然256の倍数
     (例: 256、512)へジャンプする境界を特定する - これが実際にComb側
     だけが1回多くALIGN 256パディングを踏んでいる場所。
  3. そのALIGN境界の直前のシンボルの`plain_addr mod 256`(=m)と、
     安定していたdelta値(=d、通常171のように256未満)から、
     「区間内で正味何byte削減すればdeltaが256を跳び越えずに済むか」を
     逆算する: `(m + K) mod 256`が`[256-d, 255]`の範囲(=`[1, 255-d+... )`
     おおよそ`K`を`(256-d)`〜`(256-d)+(255-... )`程度削れば良い、が
     符号・端点の扱いを間違えやすいので実際には「削減候補のbyte数Kを
     決め打ちで実測→plain/Combの両方の残りbyte数を再計測」して確認する
     方が確実(理論値はあくまで見積もりの参考)。
  4. 削減は「移動」ではなく「その区間内で実際に使用byte数を減らす」
     ことが必須条件(単なる場所替えでは条件を満たせない)。
  - **実際に効いた削減手段の例(Round145 follow-up)**: `SSC_FIRE_BLK0`
    のCPディスパッチチェーン(`CP n : JP Z,HANDLER`、5byte/件)のうち、
    連続したindex値が同じハンドラを指す"run"を
    `LD A,L : SUB lo : CP 個数 : JP C,HANDLER`(7〜8byte、runの個数に
    関わらず定数コスト)へ機械的に圧縮 - 35個のrunを圧縮しただけで
    実測156byte削減、Comb側の256byte超過が完全に解消した(plain/Comb
    ともに残り70byteで一致)。**この種の「index→ハンドラ」ディスパッチ
    チェーンにconsecutive-index runが複数あれば、まずここを疑うと
    低リスクかつ効果が大きい**(挙動を変えないことは、変換後のコードに
    全入力値[今回は0-255の全L値]を実際に流し込み、変換前と到達ハンドラ
    が完全一致することをエミュレータで網羅的に確認して保証すること)。
...
- **(2026-09-22、Round145 follow-up、ユーザー指示によりTODO化・未着手)**
  Ebuz左端-Mission1リセットバグ: "Stage1のEbuzで自機を画面左端で攻撃
  してるとMission1表示のスタートに戻される場合がある"。当初の調査
  (静的コード監査+背景エージェントによる20,000フレームの実機同等
  シミュレーション)では再現できず、「MISSION FAILED画面を経由する
  正規の死亡→タイトル遷移ではないか」という仮説を立てたが、ユーザー
  からの訂正("Mission FailedにはならずいきなりMission 1表示に飛ぶな
  恐らくEbuzのコードのどこかにルートがあるはず めったに起こらないが
  2回連続起こったりもした")によりこの仮説は**否定された**(正規の
  死亡なら必ずMISSION FAILEDを経由するはずだが、実際は経由せず直接
  Mission1表示へ飛ぶ、という点が矛盾)。ユーザーからは**"取り敢えず
  Todoにしとく"と明示的に指示されており、指示があるまで着手しない**。
  次に着手する際はround47/53/71の前例(z80emu.pyでは検出不可能な
  実機/BIOS依存のワイルドジャンプ系バグ)を念頭に、openMSX等の高精度
  エミュレータでの実機トレース調査が有力な次の一手になる可能性が高い。
  詳細は`tools/stage2_combined/HANDOFF.md`のRound145 follow-up参照。
...
## Round47: タイトル画面実機グリッチの根本原因特定・修正(完了済み)
(2026-09-06)

- ユーザー報告: 実機でタイトル画面が完全にグリッチ状態(画面写真添付)。
  "エミュレータでは綺麗に表示されるが実機ではグリッチ状態 TMS9918の
  スクリーン2に設定されてるか確認"。
- z80emu.pyはBIOSルーチンを全てno-opスタブとして扱うため検出不可能な
  クラスのバグと判断、`openmsx`(headless、`-control stdio`、
  `C-BIOS_MSX1`)+Xvfbによる自作デバッグ環境を新規構築して調査。
- **VDPレジスタは完全に正しくSCREEN2へ設定されていた**(ユーザーの
  懸念は否定)。実際の原因は別: **`DECOMPRESS_TITLE_BG`のリテラル
  セグメント転送に使っていた`OTIR`命令がVDPデータポート(98h)への
  ブロック転送では実機で実質機能しない**(MSXで広く知られるハード
  ウェア制約 - OTIRのバスタイミングがTMS9918の要求と合わないため)、
  という根本原因を特定。反復セグメント側(元々`OUT`+`DJNZ`の手動
  ループ)は正しく動作しており、リテラル側だけが機能しないことで
  VRAM書き込みアドレスがズレ、以後の全セグメントへ雪だるま式に
  累積して画面全体が無関係なデータで埋まっていた。
- **修正**: `OTIR`を反復セグメント側と同型の`OUT (n),A`+`DJNZ`手動
  ループへ書き換え。openMSXデバッグ環境で、修正前に不一致が確認
  されていた全ポイント(高エントロピーなカラーテーブル領域含む)を
  再測定し、**全て期待値と完全一致**することを確認済み。全回帰
  `run_all.py` **1397 passed/0 failed**、`title_test.py`(25件、
  ロジック無変更のため無改修で全PASS)、`verify_comb.py`確認済み。
  3ROM再ビルド・標準方針によりComb ROMのみ送付。詳細はHANDOFF.mdの
  Round47参照。
- **教訓(重大)**: MSXの実機プログラミングでは`OTIR`/`INIR`/`OTDR`/
  `INDR`のようなZ80のブロックI/O命令をVDPのデータポート(98h)への
  連続書き込みに使ってはならない - 通常の`OUT (n),A`をループで回す
  方式のみが信頼できる。この制約はz80emu.pyでは再現されないため、
...
- ユーザー報告2件: "ステージ2のBGMが壊れてる まともに鳴らなくなった"
  "ETankの弾の判定が大きすぎる...この場合はBGは表示だけでコリジョンは
  別に"。
- **BGM**: 送付済みComb ROMの実バイナリでDEFEATを2ループ分(音程+
  音量/エンベロープ+デューティ双方)再生させMIDI由来の正解データと
  完全一致(不一致0件)を確認、RAM衝突・前方参照バグ・スタック安全性も
  全て再確認したが、ロジック上のバグは発見できなかった。**根本原因
  未特定のまま、実機での具体的症状(無音/音程異常/途中で止まる等)の
  追加情報待ちで保留**(次に再開する際はHANDOFF.md Round49参照)。
- **EtankBullet当たり判定**: ユーザー追加確認("表示は当たってない様に
  見えるが当たる")から根本原因を特定 - 当たり判定自体は元々生の
  ETANK_BULLET_X/Yを使う8x8 AABBだったが、実際の描画は
  `HORMING_BG_CELL_ADDR`が`X>>3`/`Y>>3`で8px単位に切り捨てた側の
  BGセルにしかならないため、判定側の原点が見た目のセルより最大7px
  自機に近い側へずれ、「見た目は重なってないのに被弾する」不具合に
  なっていた。`CHECK_ETANK_BULLET_VS_TANK`内でX/Y読み込み直後に
  `AND 0F8h`を追加し、判定原点を表示と同じ8px境界に揃えて解消
  (+4byte)。新規回帰テスト3件(自己検証済み)。全回帰`run_all.py`
  **1431 passed/0 failed**。Comb ROM再ビルド・`verify_comb.py`健全性
  確認の上、標準方針によりComb ROMのみ送付。詳細はHANDOFF.mdの
  Round49参照。
- **保留**: BGM調査は継続中(上記参照)。`CHECK_FLYER_LASER_VS_TANK`/
...
EBULLET位置/コリジョン修正)(2026-09-06、完了済み・実機フィードバック
待ち)

- ユーザー発言3件: (1)"クリアBGMの最初の方って多分無音になってると
  思うんで発音までの無音部分をカットして"、(2)"ステージ1の敵弾(横棒
  レーザー)が敵との位置が上すぎるんで8px下げて"、(3)"判定も大きい様に
  感じる 多分上下2pxしか無いはずだけど コリジョンはそうなってるか?"。
- **StageClear先頭無音カット**: 3パート共通の先頭REST長の最小値
  (180tick)だけを全パートから一律に削り、3パート間の相対タイミング
  (メロディがbass/harmonyより13tick遅れて入る編曲上の関係)は保持。
  0tickになったパート(bass/harmony)は先頭REST行自体を削除
  (duration=0の行はTIMERへのDEC Aで0xFFへアンダーフローする実害バグに
  なるため)。データサイズ変化に伴いStage1側のSTAGE_CLEAR_CHA_BASE
  以降の全RAMアドレス・STAGE_CLEAR_TOTAL_TICKS(500→320)・Title側LDIR
  転送長を再計算・更新。
- **EBULLET(敵弾/横棒レーザー)発射位置+8px**: 全5呼び出し元が発射元
  エネミーの生Yをそのまま渡していた設計を踏まえ、`SPAWN_EBULLET`
  自身1箇所に+8を追加するだけで全パターンに一括対応。
- **EBULLETコリジョンを16x16→16幅x2高へ**: `EBULLET_PATTERN`の実際の
  絵(上半分2行のみの横バー)に合わせ、新規`PLAYER_HIT_BOX_EBULLET`
  (X:16px幅のまま/Y:スプライト原点+2から2px高)を追加し`PDC_CHECK_
  EBULLET`をこちらへ差し替え。
- 自己検証(2)(3)とも一時的にASMを元へ戻してテストが正しくFAILする
  ことを確認済み。`verify_enemy_bullets.py`のY期待値更新+
  `verify_player_damage.py`に新規2件、`verify_stage1_bgm.py`の
  StageClear関連テスト1件のpoke漏れも発見・修正。全回帰`run_all.py`
...
## Round55: Round54「row0-19限定塗り」理論の撤回・原状回復(2026-09-06、
部分対応・地形ノイズとビー音の真因は依然未特定)

- ユーザーから新スクリーンショット(地形・敵は正常だがrow20-23
  [ground scroller帯]に青白い斑点ノイズ)と共に強い叱責: "何だその
  処理手順 スクショ見ろ 無茶苦茶じゃねえか 全く修正されてねえよ
  タイトル、Mission1表示、ステージ1の手順で表示してくだけだろうが
  変な手順でやるからMission2もおかしくなってるだろうが 音も止まって
  ねえし 完全にデグレ なんでスクロールを避ける必要がある Mission2は
  その手順で問題なく動いてるだろうが"。
- **重大な反省**: Round54はレンダリングによる視覚確認(この
  プロジェクト自身がRound36-14 follow-up#4で確立した標準手順)を
  一切行わずz80emu単体テストだけで「直った」と判断して送付していた。
  これが今回の失敗の直接原因。
- Round54の「DRAW_MISSION_SCREENの生VRAM書き込みがNAMEBUF/PREVBUF
  キャッシュを永久にデズィンクさせる」という理論を、(1)NAMEBUFが
  毎フレーム完全再計算される一時バッファであるとコード読解で確認、
  (2)round53着手前コミット(2d40808)と現行コードでA/Bテスト
  (2500フレーム分のground scroller VRAM出力を比較)し**完全一致**を
  確認、の2点で完全に撤回。ユーザーの指摘が技術的に正しかった。
- `DRAW_MISSION_SCREEN`の黒埋め範囲をRound54の640byte(row0-19限定)
  からRound53以前・Mission2と同じ768byte(画面全体)へ復元。PSGチャン
  ネルA無音化・`ERASE_MISSION_TEXT`(いずれもRound54由来の別修正)は
  維持。`tools/verify_stage1_mission_screens.py`のRound54固有
  アサーション2件を768byte全域判定へ戻した(36→35件)。
- **正直な現状報告(未解決)**: (1)地形ノイズ帯の真因は依然未特定
  (z80emuでは検出不可能な実機/高精度エミュレータ専用の不具合の
  可能性が高いが未確証、768byte版に戻せば消えるかも未検証)。
  (2)"ビー"音が止まっていないという報告も未再調査(Round54のチャン
  ネルA無音化だけでは実機で解消していない、R8以外の経路を疑う必要
  あり)。
- 全回帰: Stage2側`run_all.py` **1447 passed/0 failed**(無変化、
  今回combined_test.asmは無編集)。Stage1側`verify_stage1_mission_
  screens.py`(35)/`verify_player_damage.py`(58)/`verify_stage1_
  bgm.py`(70)/`verify_enemy_bullets.py`(49)、全てPASS。3ROM再
  ビルド・`verify_comb.py`健全性確認の上、標準方針によりComb ROMの
  み送付。詳細はHANDOFF.mdのRound55参照。
- **教訓(重大)**: 見た目・音に関わる修正は、z80emu単体テストが
  全てPASSしていても「直った」と報告・送付しない。必ずStage1用の
  VRAM→PNGレンダリングスクリプト(現状未作成、`tools/stage2_
  combined/render_check.py`をベースに新規作成する必要がある)で
  実際に画面を再現してから確認すること。
- **保留(次回優先対応)**: (a)Stage1用render-checkスクリプトの新規
  作成が最優先(これが無いままでは今後も同種の失敗を繰り返すリスクが
  高い)。(b)地形ノイズ帯の真因調査(openMSX等の高精度エミュレータでの
  実機再現アプローチが有効な可能性、Round47のOTIR教訓を参照)。
  (c)"ビー"音が止まらない件の再調査。
...
- ユーザー叱責: "ただMission表示するだけでよ ステージとMission表示は
  ...
  なってんだろうが 3秒待たされてんだからよ"。
- **根本原因**: Mission1表示ブロックがINITの中盤(背景/スプライトVRAM
  転送・敵プール初期化等"ステージ本編の初期化"の真っ只中)に挟まって
  いるだけで、独立フェーズとして分離されていなかった。加えて
  `INIT_BGM`が`BGM_MUTED`を0(即アンミュート)で初期化していたため、
  背景/スプライトVRAM転送ループの合間にある無数の短いEIウィンドウの
  どれかでH.TIMIが発火するたび、PSG R7ミキサーがまだ未設定の状態の
  ままchB/chCへBGM書き込みが起こりうる構造だった。
- **対応3点**: (1) Mission1表示ブロック(font読込+表示+3秒待ち+消去)
  を`CALL INIT32`の直後、INITの中で最も早い位置へ移設 - 以後、border
  色設定〜背景塗り〜敵プール初期化〜PSG R7ミキサー設定までの"ステージ
  本編の初期化"が丸ごとMission1の後に実行される構造に変更(副産物として
  Round54で誤診断していた"初期画面が描画されてない"の真因[背景描画が
  Mission1の黒塗りより前に終わっており、消去時に9byteのメッセージ領域
  しか復元されないため元の背景が戻らない]も解消)。(2) `INIT_BGM`の
  `BGM_MUTED`初期値を0→1(ミュート開始)に変更、既存の`MUTE_BGM`/
  `UNMUTE_BGM`を再利用しINITの本当に最後(EI/HALT/JP MAINLOOP直前)で
  1回だけUNMUTE_BGMを呼ぶ設計に変更 - これによりMission1表示中を含む
  INIT全体を通じて、たとえH.TIMIが発火してもBGMは絶対に鳴らない構造的
  対策とした。(3) font読込を早めた副作用で発覚した新規バグ(COLORDATA
  の一括ロードがMission1フォントの色を巻き込んで上書き)を、COLORDATA
  ロード直後にフォント色を再書き込みする形で修正(ステージクリア時の
  MISSION2が同じフォント・色を再利用するため必要)。
- 新規回帰テスト3件(INIT_BGM単体でBGM_MUTED=1・実boot()でMAINLOOP
  到達時BGM_MUTED=0[UNMUTE_BGM実行済み]・生トレースでDRAW_MISSION_
  SCREEN開始時点で既にBGM_MUTED=1)、既存テスト1件を新設計に合わせて
  書き換え。全て自己検証(修正一時取消→FAIL確認→復元→PASS確認)済み。
  全回帰: Stage2側`run_all.py` **1447 passed/0 failed**(無変化)。
  Stage1側`verify_stage1_mission_screens.py` **38 passed**(35→38)・
  `verify_player_damage.py` 58・`verify_stage1_bgm.py` 70・
  `verify_enemy_bullets.py` 49、全てPASS。3ROM再ビルド・
  `verify_comb.py`健全性確認の上、標準方針によりComb ROMのみ送付。
  詳細はHANDOFF.mdのRound56参照。
- **保留・実機フィードバック待ち**: 地形ノイズ帯の真因調査は今回の
  スコープ外(ユーザーの今回の指摘はMission1周りに限定)、引き続き
...
- ユーザー報告: "ステージ1だが 敵弾のレーザーが1回の自機への被弾で
  複数回ダメージ食らってる場合がある"。
- **根本原因**: `EBSD_UPDATE`(TYPE_ENEMY4=Fighter/Enemy7/E1)には、
  独立した2つのEBULLET発射条件が存在する - "Y軸一致発射"
  (align-fire)と"斜めドッジ発動時のランダム発射"(dodge-fire)。
  既存コード自身のコメントが"independent of (and can co-occur
  with)"と明記していた通り、同一フレームで両方の条件を満たすと、
  完全に同一座標から2発のEBULLETが同時発射されることをシミュレー
  ションで確認した(2つの独立したEBULLET_POOLスロットが全く同じ
  (X,Y)でアクティブになることを直接確認済み)。BARRIER_IFRAMES
  (60フレームの無敵時間・弾自体もヒット時に消費)により単純な
  シナリオでは2回目のヒットまでは再現できなかったが、"1体の敵から
  2発が完全に重なって発射される"こと自体が明確な構造的バグであり、
  これが実機報告の直接原因である可能性が最も高いと判断。
- **修正**: 新規1バイトRAM`E4_FIRED_THIS_FRAME`(0E668h)を追加。
  align-fireが発火した際に1を立て、dodge-fire側は
  `DECIDE_FIRE_SHOOTER`自体(カウントダウン消費)は無条件に呼びつつ、
  実際のSPAWN_EBULLET呼び出し直前でこのフラグを確認し、既に発射
  済みならスキップ - ダイブの動き・カウントダウン消費には一切影響
  せず、弾の二重発射だけをピンポイントで防ぐ最小修正。INIT時の
  明示的ゼロクリアも追加。
- 新規回帰テスト7件を`tools/verify_enemy_bullets.py`に追加、自己
  検証(修正一時取消→FAIL確認→復元→PASS確認)済み。全回帰: Stage2側
  `run_all.py` **1447 passed/0 failed**(無変化)。Stage1側
  `verify_enemy_bullets.py` **56 passed**(49→56)・
  `verify_player_damage.py` 58・`verify_stage1_bgm.py` 70・
  `verify_stage1_mission_screens.py` 41、全てPASS。3ROM再ビルド・
  `verify_comb.py`健全性確認の上、標準方針によりComb ROMのみ送付。
  詳細はHANDOFF.mdのRound58参照。
- **保留・実機フィードバック待ち**: シミュレーションでは実際に複数回
  ダメージが入るシナリオそのものは再現できなかった(BARRIER_IFRAMES
  の無敵時間ガードが強力なため)。実機で修正後も同じ症状が続くようで
  あれば、EBULLET以外の被弾経路も含め追加調査が必要。
...
- ユーザー報告: "フリーズは解消された スケジュールの問題はどうなった
  ...
  "フリーズは解消された"で確認取れた)。
- **E2A_ACTIVE/E2B_ACTIVE(Zigzag=Enemy2編隊A/Bの生存フラグ)のINIT時
  未クリア仮説を立て、一度は修正・テストを実装したが、2つの検証
  (①INIT冒頭に既存の0xE600-0xE663/0xE680-0xE6E3の一括ゼロクリア
  ブロックが実は既にこの2変数を含めてクリアしていたと判明[grepでは
  見落としていた]、②Comb構成での実プレイシミュレーション[Enemy2
  編隊生存中に強制ゲームオーバー→title→Stage1再起動→tick260まで
  進行]で1周目・2周目とも全く同じタイミングでZigzagが正常にスポーン
  することを確認)で誤りと判明し、修正は撤回した。**真因は依然
  未特定。**
- **副産物**: 同じ監査で`DFL0-2_ACT`(ボス偏向弾3スロット)がボス
  スポーン時の一括初期化ブロックに含まれていない実バグ(E2A/E2Bとは
  無関係)を発見・修正(ボス戦中のゲームオーバーで次のボス戦に偏向弾
  の残骸が持ち越されうる潜在バグ)。
- 新規`tools/verify_spawn_schedule_restart.py`(8件)。全回帰: Stage2側
  `run_all.py` **1459 passed/0 failed**(無変化)。Stage1側4ファイル
  全てPASS(無変化)。3ROM再ビルド・`verify_comb.py`健全性確認の上、
  標準方針によりComb ROMのみ送付。詳細はHANDOFF.mdのRound61参照。
- **保留・要追加情報**: 今回のシミュレーションは「Enemy2編隊生存中に
  死亡→即座にtitleへ復帰→即座に2周目開始」という最短経路のみ検証
  (実時間13秒の待機や複数回死亡の累積影響は未検証)。Round47/53と
  同型の「z80emu.pyでは検出不可能な実機/BIOS依存バグ」である可能性も
  残る。ユーザーからより具体的な再現条件(正確なtick数or経過時間・
  何回目のゲームオーバー後か・A/Bどちらのボタンで再開したか)の追加
  情報が得られれば次の調査の手掛かりになる。

## Round62: 上記の真因確定・修正(完了済み・実機フィードバック待ち)
(2026-09-07)

- ユーザー追加情報"正確なTickなんて分からんが スケジュール上でみれば
  235から304まで エネミー4までの敵がリスタートでは出ない 普通に
  考えてリスタートでの初期化ミスだろ"を受け再調査。tick235-304
  (SPAWN_THRESHOLDS index51-69)はSPAWN_E4B(TYPE_ENEMY1_LOOK="Wave")/
  SPAWN_SIMPLEのみでSPAWN_E2を含まないため、Round61のSSC_BUSY_E2
  永久停止理論では説明できないと判明。実プレイシミュレーションで
  SPAWN_NEXT_INDEX自体は1周目・2周目で完全一致(スケジュールは正しく
  進む)と確認した上で調査した結果、**真因は`SIMPLE_PATTERN_USED`
  (Enemy1/Enemy5/Enemy4Bが共有する6枠だけの物理VRAMパターンプール)
  がSPRITE_USEDと同型の"free-list"方式でありながらINITで一度も
  明示的にクリアされていなかった実バグ**と判明。ゲームオーバーが
  この6枠のいずれかを使用中の敵が画面上に残っている瞬間に発生すると
  枠が永久に"使用中"のまま残留し、次の周回でプールが目減りした
  まま再スタート、tick235-304に13件も密集するSPAWN_E4Bバーストが
  ALLOC_PATTERN_SLOT失敗で次々ドロップされていた(スケジュール自体は
  正常に進みつつスポーンだけ静かにdropされる設計のため異常が
  見えなかった)。SPRITE_USED直後に同型のゼロクリアを追加して解消。
  新規回帰テスト4件(`tools/verify_spawn_schedule_restart.py`、
  8→12件)、全回帰・3ROM再ビルド・verify_comb確認の上、Comb ROMのみ
  送付。詳細はHANDOFF.mdのRound62参照。
- **保留・実機フィードバック待ち**: 実機での症状解消は次回フィード
  バック待ち。最初の報告(Zigzag消失)と2回目の報告(tick235-304の
  Enemy4系消失)は別の症状だった可能性があり、Zigzag自体の消失が
  別途再現するようであれば追加調査が必要。

## Round63: タイトル確認音+自機爆発音(ボス方式に統一)+Stage1自機死亡
落下演出+Stage2容量再確認(完了済み・実機フィードバック待ち)(2026-09-07)

- ユーザー指示4点に対応。(1) タイトル画面ボタン押下で確認ビープ音を
  追加(チャンネルB、周期150・音量12→1減衰、R7は無変更)。(2) 自機爆発
  音をボスの死亡演出と同じ「ポップごとに毎回鳴る」方式に統一 -
  `PEUA_TRY_SPAWN`の各スポーン成功時に`SOUND_DESTROY`を追加(従来は
  死亡の瞬間1回だけだった)。(3) Stage1自機死亡演出を新設: 被弾即座に
  MISSION FAILED表示していたのを、まず操作無効の上PLAYER_DEATH_FALL_
  DURATION(45フレーム≒0.75秒)かけて右斜め下へ等速落下しながら爆発
  (既存のPLAYER_EXPL_POOLバーストが自動追従)、完了後に初めてMISSION
  FAILEDテキスト+GAME_OVER_SEQ状態機械を起動する2段階へ再設計。
  (4) Stage2 ROM容量を再確認 - 現在も32767/32768バイト(残り1byte)、
  ALIGN 256の整列コストは現時点でゼロ(round60の「ALIGN相殺」は
  あの1回限りの巡り合わせ)。他バンクへの退避はホットパスコードには
  実行時制約上不可能、GAME_TICK_DISPLAYはround60で削除済みのため
  再削除できるものは無し(コード変更なし、確認結果の報告のみ)。
- 新規回帰テスト: `title_test.py`(27→31件)、
  `verify_stage1_mission_screens.py`(65→71件)、`verify_comb.py`の
  GAME_OVER_SEQ統合テストを新設計に更新。全回帰: Stage2側`run_all.py`
  1459 passed/0 failed(無変化)。Stage1側4ファイル全てPASS。3ROM
  再ビルド・verify_comb確認の上、標準方針によりComb ROMのみ送付。
  詳細はHANDOFF.mdのRound63参照。
- **保留・実機フィードバック待ち**: 落下速度・時間・確認音の音程/
  減衰カーブはいずれも未調整の初期値。落下演出が画面外(255クランプ)
  に到達した際の実機での見た目は未検証。

## Round63 実機フィードバック対応: 自機を画面外へ完全非表示化+MISSION
FAILEDの毎フレーム再描画(完了済み・実機フィードバック待ち)(2026-09-07)

- ユーザー発言2点に対応。(1) 落下完了時、PLAYERX/PLAYERYを明示的に
  「(255,199)、実際の描画Y=191=ENEMY_HIDE_Y」という既存の非表示
  コーナー慣習へ強制するよう変更(自機の描画コードはPLAYERY-8を実際の
  スプライトYとして書くため199で逆算)。ゲート構造もGAME_OVERを
  マスターゲートとする2階層に再設計し、落下完了後も操作無効が永続する
  よう修正(旧実装は45フレーム後に通常入力へ戻ってしまう抜け漏れが
  あった)。(2) DRAW_GAMEOVER_TEXTを「テキストのみオーバーレイ」方式の
  ままUPDATE_GAME_OVER_SEQUENCE(既に毎フレーム無条件に呼ばれている)
  から毎フレーム再描画するよう変更、他のBG書き込みに上書きされても
  次のフレームで自己修復する。
- `verify_stage1_mission_screens.py`を71→75件へ拡張、Stage1用
  render-checkで死亡完了直後のフレームを実際にレンダリングし、
  MISSION FAILED表示+自機本体の完全非表示を視覚確認済み。全回帰:
  Stage2側1459 passed/0 failed(無変化)、Stage1側4ファイル全てPASS。
...
## Round65: Stage1自機死亡落下を自然な画面外消失方式へ・自機爆発に
イエロー追加(完了済み・実機フィードバック待ち)(2026-09-07)

- ユーザー指示: "ステージ1の自機爆発で斜め下に落下したらそのまま画面外に
  消えるように変更 で、ステージ1と同じようにイエローを加えた爆発に"。
  従来の固定45フレームタイマー+強制テレポート方式を廃止し、PLAYERYが
  実際に画面外の値(199=ENEMY_HIDE_Yの逆算値)へ到達するまで等速で
  落下し続ける方式へ再設計(開始位置からの距離がそのまま落下時間になる
  自然な連続落下)。オーバーフロー時のクランプ先も255(画面内の誤った値)
  から199(画面外、完了扱い)へ修正。`PLAYER_DEATH_FALL_TIMER`/
  `_DURATION`は削除。自機爆発の色サイクルに新規`SPR_YELLOW(0Bh)`を
  追加し白/ライトレッド/イエローの3色化。`tools/verify_stage1_mission_
  screens.py`77件(+2、落下時間が距離に比例することの直接検証含む)・
  `verify_comb.py`更新、全回帰Stage2側1459 passed/0 failed(無変化)、
  Stage1側4ファイル全てPASS。Comb ROM再ビルド・verify_comb.py確認の
...
全面差し替え+タイトル確認音v3実装完了(2026-09-07)

- ユーザー確認: "タイトル音はそれで良い ただしオクターブ下げて
  デューティ比50%で 爆発もそれでOk"。
- **タイトル確認音**: `PLAY_CONFIRM_BEEP`をテーブル駆動(53行)へ
  全面書き換え。チャープ上昇→ホールド→fadeout→ブザー下降→fadeout、
  全周期オクターブ下げ、各行を前半(音量あり)/後半(音量0)に分割して
  デューティ比50%を実現、53行×2回再生。title_test.py側でPSG書き込み
  列をPython参照実装と完全一致検証(424回)、既存の「button press」
  テストのステップ上限を100,000→2,000,000へ拡大(新ビープの所要
  ステップ数に対応)。31 passed。
- **敵撃破爆発アニメーション**: `TRIGGER_EXPLOSION`(単一セル2フェーズ
  フリッカー)を全面書き換え。実装前にGIFプレビュー(元絵の帯分解+
  コンタクトシート+実アニメGIF)で認識確認してから着手(ユーザー
  指示"実装前にGIFで見せてくれ")。
  - **重要な自己発見**: ユーザーの当初想定"Enemy3のパターン2/3を
    流用"は、その2コードが最大64体同時生存するEnemy3自身の生存中
    パルスアニメの一部であり再利用すると実害バグになると判明、
    代わりに「元の爆発パターン(ANIM_PATTERNS)自体を削除して空きに」
    という要求通り、削除跡地自身(旧ANIM2_BLUEのgroup、fg赤/bg青、
    COLORDATA無変更で流用可)を使う設計に変更。
  - JSON解析の結果、必要な絵柄は実は「2行バー」「4行バー」の2種類
    のみ(5タイル中3つが同一ビットマップと判明)。`EXP_CODE_THIN`
    (120)/`EXP_CODE_THICK`(121)、旧512byte→16byteへ削減。
  - フレーム1=中心列のみ/フレーム2=3列(THIN,THICK,THIN)/フレーム3=
    1列空けて外側2列(中央列は背景復元)、画面左端でクランプ。新規
    RAM(`EXPLOSION_SAVED_CM3`、4列目の退避用3byte)は既存ANIM_BASE
    スロットのstrideを拡張せず全く別の安全な領域(0xF237)に新設
    (ANIM_RR以降の手書き連番RAMアドレス群を巻き込むカスケードを
    回避)。
  - 地上敵の水平打ち破壊も同じ経路に統一(旧来の行別4色分岐を撤廃、
    常に同一色 - 自機ショットと同じ簡略化)。
  - 新規`tools/verify_explosion_anim.py`(28件)。VRAM→PNG(GIF)
    レンダリングで実際のアニメ再生を視覚確認、送付済み。
- 全回帰: Stage2側`run_all.py` **1482 passed/0 failed**(無変化)。
  Stage1側5ファイル全てPASS(`verify_explosion_anim.py`28件新規)。
  `verify_comb.py`全チェックPASS。3ROM再ビルド・標準方針によりComb
  ROMのみ送付。詳細はHANDOFF.mdのRound69 follow-up2参照。
- **保留・実機フィードバック待ち**: タイトル確認音v3・新爆発アニメの
  実機での聞こえ方/見え方は次回フィードバック待ち。

## Round70: Stage2自機爆発1個ずつ+毎回サウンド化・Stage1ステージクリア後
ノイズ再発(3度目)の真因修正・新爆発アニメ半減・ボス着地時の反射弾残留
バグ修正・タイトル確認音の枠色フラッシュ実装(2026-09-07、完了済み・
実機フィードバック待ち)

- ユーザー: "ステージ2の自機爆発は4つ爆発を同時に飛ばすんじゃなく1個
  ずつバラバラにだ...でその1回毎にサウンドだ...ボス爆発がそうなって
  んだろうが で、またステージ1クリア後の音が止まってない 何回やるん
  だよクソが で、タイトルでの音だした時の枠描画はどこいったんだよ"、
  続けて"新爆発の表示時間は半分でいい 長すぎ で、ステージ1のボスの
  マテリアライズ中のショットの反射弾が残ってる 前はそんな事なく消えて
  た"の5件に対応。
- (1) Stage2自機爆発: 実際のボス撃破演出(`BOSS_EXPL_UPDATE`/
  `BEU_FIRE`)を読み直し「毎回新しい位置に1個ポップ+毎回SOUND_DESTROY+
  短く待って次」という高速連続ポップが正しい参照実装と確認、
  `gameover_bank.asm`の`GO_EXPLOSION_SEQUENCE`を同型へ全面書き換え。
  (2) Stage1クリア後ノイズ(3度目の再発): `SOUND_UPDATE`は
  `STAGE_CLEAR_ACT==0`の間だけ呼ばれるため、`TRIGGER_STAGE_CLEAR`が
  ACTを1にした瞬間からR8は誰にも更新されなくなり、flyawayのエンジン
  音の最後の値が固まって鳴り続けていた。`TRIGGER_STAGE_CLEAR`自身が
  ACT=1にする直後にR8=0を明示的に書き込み解消。(3) 新爆発
  (`ANIM_FRAME_LEN`)を8→4へ半減。(4) ボス着地時のDFL0-2(偏向弾)残留:
  DFL用固定スプライトスロット9-11は「materialize中に消え切っている
  前提で着地後の8機周回ポッド(スロット6-13)と重複可能」設計だったが、
  着地の瞬間に生存中の偏向弾が残るとポッドとスロットを奪い合い残留する
  実バグと判明、新設`DFL_FORCE_CLEAR`を着地処理の先頭に追加して解消。
  (5) タイトル確認音の枠色フラッシュ: 承認済みcandidate06は元々PSG+
  VDPボーダー明滅の対設計だったが枠色側の実装が抜けていたと判明。
  `tools/z80emu.py`にVDPレジスタ書き込み記録(`vdp_regs`)を新規追加し、
  `title_test.asm`にIXベースの`BORDER_TABLE`(53byte、REDGRAD=
  [黒,暗赤,中赤,明赤,中赤,暗赤,黒]を`row*7//53`で配分)を追加、確認音
  の各行と同期してVDP R7へ書き込む。
  全回帰: Stage2側`run_all.py` **1472 passed/0 failed**。Stage1側
  `verify_stage1_bgm.py` 71・`verify_boss_dfl_clear.py`
  **10 passed(新規)**、他全てPASS。`title_test.py` **35 passed**。
  `verify_comb.py`全チェックPASS。3ROM再ビルド・標準方針によりComb
  ROMのみ送付。詳細はHANDOFF.mdのRound70参照。
- **保留・実機フィードバック待ち**: 上記5件全て実機での実際の見え方/
  聞こえ方は次回フィードバック待ち。(4)のDFL修正は構造的に妥当な原因
  として特定・修正したが100%の確証はなく、症状が続く場合は追加調査
  が必要。
...
- ユーザー自身の診断: "ステージ1で画面が壊れる原因が分かった 爆発処理で
  ...
  解決する"。
- **根本原因**: 死亡落下が始まると方向入力の読み取りが丸ごと迂回される
  が、これによりJOY_TRIG(GTTRIG結果のRAM変数)を毎フレーム更新する
  スキャンも一緒に迂回され、死んだ瞬間の値のまま"凍結"する。発射
  チェックは独立して`PLAYER_FLYAWAY`しか見ておらず、凍結した"押し
  っぱなし"状態のJOY_TRIGに反応して死亡落下中ずっと新規弾を発射し
  続けていた。落下で異常値(150〜199)になった生の`PLAYERY`から新規弾の
  行番号を計算するため、24行分しかない`ROWADDR_LO/HI`テーブルを範囲外
  indexで読み、直後の`PATTERNS`データから拾った値がそのまま不定の
  VRAM書き込みアドレスになる(実証済み: PLAYERY=199でROW=25、
  カラーテーブル近辺まで書き込み先が飛ぶ)。
- **修正**: 発射チェック先頭に`GAME_OVER`ガードを追加、GAME_OVER=1の
  間は発射も方向入力と同時に完全停止。新規回帰テスト2件(実際の
  frozen-JOY_TRIGシナリオを正しく再現、当初のテストは無意味だったと
  自己発見・修正)。全回帰: Stage2側`run_all.py` **1472 passed/
  0 failed**(無変化)。Stage1側`verify_stage1_mission_screens.py`
  **86 passed**、他全てPASS。`verify_comb.py`全チェックPASS。3ROM
  再ビルド・標準方針によりComb ROMのみ送付。詳細はHANDOFF.mdの
  Round71参照。
- **保留・実機フィードバック待ち**: 実機での画面破損解消は次回
  フィードバック待ち。round55保留の「地形ノイズ帯の真因」がこのバグと
  同一かは未確定。

## Round72: Stage2自機爆発の音が鳴りっぱなしのままMISSION FAILEDになる
バグ修正(2026-09-08、完了済み・実機フィードバック待ち)

- ユーザー報告: "ステージ2の自機爆発で音が出っぱなしでMission Failedに
  なってる 消してからゲームオーバーにしろ 音の消し忘れ多すぎだろうが"。
- **根本原因**: `gameover_bank.asm`の`GO_ARM_BOOM`/`GO_STEP_BOOM_DECAY`
  はポップ(1回の爆発)ごとに音量15へ撃ち直してから2段(15→13→11)しか
  減衰させず、次のポップでまた15へ再武装されるため、シーケンス全体を
  通じてR8が一度も0に達しない。最後のポップ後もR8=11のまま残り、続く
  MISSION FAILEDテキスト表示中もずっと鳴り続けていた。
- **修正**: `GO_HIDE_EXPLOSION`(爆発演出の後片付け)の冒頭でR8=0を
  明示的に書き込み解消。新規回帰テスト2件、一時的に修正を取り消して
  FAILすることを自己検証済み。全回帰: Stage2側`run_all.py`
  **1474 passed/0 failed**。`verify_comb.py`全チェックPASS。3ROM
  再ビルド・標準方針によりComb ROMのみ送付。詳細はHANDOFF.mdの
  Round72参照。
- **教訓**: 「ポップごとに撃ち直し、少しだけ減衰する」ループ駆動SEは
  ループ終了時点で音量が0まで落ちきっている保証がない - ループの外で
...
順序修正)(2026-09-08、完了済み・実機フィードバック待ち)

- "鳴ってるが最初の方が自機爆発音で消えてる 爆発が終わってからMission
  Failed表示して音消してゲームオーバーサウンドだろうが"に対応。
- Round74の実装はジングル再生(H.TIMIフック設置)をINIT冒頭で即座に
  開始していたため爆発音と並走していた。ジングル開始(`GO_INIT_BGM`
  呼び出し)をMISSION FAILEDテキスト描画完了後(爆発終了後)へ移設し、
  「爆発→Mission Failed表示→ジングル」の直列順序に修正。
- 全回帰: Stage2側`run_all.py` 1488 passed/0 failed(1486→1488)。
  `verify_comb.py`全チェックPASS。Comb ROM再ビルド・標準方針により
  Comb ROMのみ送付。詳細は`tools/stage2_combined/HANDOFF.md`の
  Round75を参照。
...
## Round78: Stage1スケジュール再差し替え(549件、Nブロック汎用化)+
Enemy2ジグザグ合体Y段違い+Enemy6速度半減、Stage2自機爆発の自機非表示
タイミング修正(2026-09-12、完了済み・実機フィードバック待ち)

- Stage1スケジュールを`Schedule_3.json`(549件)へ再差し替え。397件
  だったRound77時点のH=0/1の2ブロック構成では549件(最大index548、
  H=0/1/2の3値必要)を表現できないと判明し、`SSC_FIRE`のブロック分岐を
  「Nエントリから必要なブロック数を自動計算しCPチェーンを生成する」
  汎用形に書き直した(以後256の倍数を跨ぐ差し替えでも人力調整不要)。
  一見index438付近で長時間進まないように見えた挙動はSSC_BUSY_E2による
  正常な累積遅延(最終的にGAME_TICK1121で完全消化・ボス正常スポーン)
  と確認、バグではない。
- Enemy2(ジグザグ)合体演出: 合体シーケンス中(state0-5)だけU0(1機目)
  をY-8、U2(3機目)をY+8にオフセット(U1=2機目は無変更、hit-test用Yも
  同期)、state6(ドリフト開始=合体完了)は元々3機とも同一Yへ戻す実装
  だったため無変更のまま「合体後は変更無し」を満たす。A/B両インスタンス
  に適用。
- Enemy6速度半減: `ENEMY6_STEP_FRAMES`を1→2。
- Stage2自機爆発: Round76で着手しユーザーに却下されたBG化を全面撤回
  した後の正しい最小修正 - 描画方式(スプライトポップ)は変更せず、
  描画先スロットをTANK自身の0-3からEtank専用スロット(24-25)へ変更
  するだけで「爆発中は自機表示・終了後に自機と破片を両方隠す」を実現。
  `gameover_bank_test.py`を新スロット配置に合わせ全面改訂・自己検証済み。
- 全回帰: Stage2側`run_all.py` **1502 passed/0 failed**。Stage1側
  既存検証群全てPASS。Comb ROM再ビルド・`verify_comb.py`全チェック
  PASS。詳細はHANDOFF.mdのRound78を参照。
- **保留・実機フィードバック待ち**: 549件スケジュールのペーシング・
  Enemy2 Y段違い演出の見え方・Enemy6速度半減の難易度感・自機爆発
  タイミング修正の実機での見え方は、いずれも次回フィードバック待ち。
...
- ユーザー報告(Round78のEtankスロット流用直後): "ステージ2の自機爆発
  ...
  範囲だけなんで"。
- 根本原因: TMS9918のhwスプライト優先度はスロット番号が若いほど手前に
  描かれる。Round78で爆発をEtank流用スロット(24-25)へ移した結果、
  TANK自身のスロット0-3の方が若いため爆発がTANKの裏に隠れていた。
- 修正: `tools/gameover_bank/gameover_bank.asm`にTANK自身のATTRIBUTE
  内容(スロット0-3)を新設`NEW_TANK_SPR_BASE_SLOT`(28-31)へVRAM上で
  1回だけ退避する`RELOCATE_TANK_SPRITE`(手動IN/OUT、OTIR等は不使用)を
  新設しINIT冒頭で呼ぶ。空いた最若スロット0-3を爆発側`EXPL_SPR_BASE_
  SLOT`に割り当て直し(24→0)、Etank由来の「2スロットまで」制約も消滅
  したため`EXPL_SPR_SLOT_COUNT`を2→4へ復元。範囲拡大は`GO_POP_JITTER`
  を16→32(-8..+7px→-16..+15px)。
- 自己発見バグ: `SPRATR+NEW_TANK_SPR_BASE_SLOT*4`という書き方が、この
  アセンブラの演算子優先順位なし・左から右への逐次評価という既知の罠
  (Round36-14 follow-up#8と同型)を再び踏み、無関係なアドレスへコピー
  していた。新規回帰テストで即座に検出、`NEW_TANK_SPR_BASE_SLOT*4+
  SPRATR`の順へ修正して解消。
- 視覚確認: このバンクの独立性ゆえrender_check.pyは使えず、スクラッチ
  スクリプトでチェッカーボード柄のTANKスタンドイン+単色の爆発パターンを
  VRAMへ手動注入し実INITフローを実行、爆発がTANKを完全に覆い隠す
  (=手前に描かれる)ことをPNGレンダリングで視覚確認済み。
- `gameover_bank_test.py`に新規3件追加。全回帰: Stage2側`run_all.py`
  **1507 passed/0 failed**(1502→1507)。Comb ROM再ビルド・
  `verify_comb.py`全チェックPASS。標準方針によりComb ROMのみ送付。
  詳細はHANDOFF.mdのRound79を参照。
