...
- 全回帰テスト: `cd tools/stage2_combined/tests && python3 run_all.py`
  - **実測所要時間: 約6分**(629テスト。2026-08-25の高速化前は約20分だった - 経緯は下記)。
    出力量自体は36行/1.5KB程度でトークンコストは低いが、ウォールクロックはまだ軽くない。
...
### 高速化の経緯(2026-08-25、629 passed/0 failedを維持したまま約3.2倍)
...
**未実施(検討したが見送り)**: PyPy実行 — このセッションは使い捨てコンテナのため、ここで
`apt install`しても次回セッションには残らない。恒久導入するなら環境のセットアップスクリプト
側に組み込むか、ユーザーのローカル環境で実行する必要がある(ユーザーへの確認待ち)。
オブジェクトファイル・リンカ分離 — アセンブル自体は0.2〜0.5秒でボトルネックではないため
効果なしと判断済み(上記「ビルドコマンド」の項参照)。
